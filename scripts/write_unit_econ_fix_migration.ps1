...
# Force repo root as current directory
Set-Location -Path (Split-Path -Parent $MyInvocation.MyCommand.Path)
Set-Location -Path ".."

# Compute next migration safely
$nums = Get-ChildItem .\supabase\migrations\*.sql | ForEach-Object {
  if ($_.Name -match '^(\d{4})_') { [int]$matches[1] }
} | Where-Object { $_ -is [int] }

$max = ($nums | Measure-Object -Maximum).Maximum
if (-not $max) { $max = 0 }

$next = "{0:0000}" -f ([int]$max + 1)
$migRel = "supabase\migrations\${next}_unit_econ_seed_and_fixes.sql"
$mig = Join-Path (Get-Location) $migRel

# SQL (clean, no stray text)
$sql = @"
begin;
...
begin
  ...
  return jsonb_build_object('ok', true, 'job_id', p_job_id, 'org_id', v_org, 'wac_days', v_wac_days, 'days_recomputed', v_days_recomputed, 'lines_updated', v_lines_updated);
end \$\$;
...
-- C) Demo seeder RPC (no SQL editor)
...
begin
  ...
  on conflict (org_id, sku, effective_start) do update set yield_qty=excluded.yield_qty, notes=excluded.notes;
...
[System.IO.File]::WriteAllText($mig, $sql, $utf8NoBom)

Write-Host "✅ Wrote migration: $mig"
