...
-- 1) Tier flag for paid premium (admin-only by logic, but tier controls availability)
alter table public.subscription_tiers
  add column if not exists paid_forecast_enabled boolean not null default false;

-- Default: keep paid off for all tiers (totally free for now)
update public.subscription_tiers
set paid_forecast_enabled = false;

-- 2) Entitlements v2 (new RPC; avoids breaking existing return types)
...
-- 3) Enforce paid flag inside create_forecast_run (signature unchanged)
...
begin
  ...
  -- Paid engine: platform admin only AND tier flag must be enabled
  if v_engine = 'paid_premium' then
    ...
    -- Free engine: approved org member required
