...
begin
  ...
  p2_self as (
    select
      ...
      mx.invoices::numeric as x, mx.avg_ticket::numeric as y, null::int as n, 2 as plot_order
    from mx where mx.org_id = p_org_id
  ),
  p2_oth as (
    select
  ...
  p3_self as (
    select
      'labor_pct_vs_margin','Labor % vs Gross Margin','Labor %','Gross Margin %',
      'self', mx.branch_id, mx.branch_name,
      mx.labor_pct::numeric as x, mx.gross_margin::numeric as y, null::int as n, 3 as plot_order
    from mx where mx.org_id = p_org_id
  ),
  p3_oth as (
    select
    ...
    from (
      select
        ntile((select b from bcnt)) over(order by mx.labor_pct) as bucket,
        mx.labor_pct::numeric as xv,
        mx.gross_margin::numeric as yv
      from mx
      where mx.org_id <> p_org_id and mx.labor_pct is not null and mx.gross_margin is not null
    ) s
    group by s.bucket
  ),
  ...
  p4_self as (
    select
      'overhead_pct_vs_margin','Overhead % vs Gross Margin','Overhead %','Gross Margin %',
      'self', mx.branch_id, mx.branch_name,
      mx.overhead_pct::numeric as x, mx.gross_margin::numeric as y, null::int as n, 4 as plot_order
    from mx where mx.org_id = p_org_id
  ),
  p4_oth as (
    select
    ...
    from (
      select
        ntile((select b from bcnt)) over(order by mx.overhead_pct) as bucket,
        mx.overhead_pct::numeric as xv,
        mx.gross_margin::numeric as yv
      from mx
      where mx.org_id <> p_org_id and mx.overhead_pct is not null and mx.gross_margin is not null
    ) s
    group by s.bucket
  ),
