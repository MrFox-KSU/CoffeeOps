...
begin
  ...
  p5_self as (
    select
      'sales_vs_gross_profit','Net Sales vs Gross Profit','Net Sales','Gross Profit',
      'self', mx.branch_id, mx.branch_name,
  ...
  p5_oth as (
    select
      'sales_vs_gross_profit','Net Sales vs Gross Profit','Net Sales','Gross Profit',
      'others', null::uuid, 'Others',
      avg(x)::numeric, avg(y)::numeric, count(*)::int, 5
    from (
      select ntile((select b from bcnt)) over(order by mx.net_sales) as bucket,
             mx.net_sales::numeric as x,
             mx.gross_profit::numeric as y
  ...
  p6_self as (
    select
      'ticket_vs_cogs_unit','Avg Ticket vs COGS/Unit','Avg Ticket','COGS/Unit',
      'self', mx.branch_id, mx.branch_name,
  ...
  p6_oth as (
    select
      'ticket_vs_cogs_unit','Avg Ticket vs COGS/Unit','Avg Ticket','COGS/Unit',
      'others', null::uuid, 'Others',
      avg(x)::numeric, avg(y)::numeric, count(*)::int, 6
    from (
      select ntile((select b from bcnt)) over(order by mx.avg_ticket) as bucket,
             mx.avg_ticket::numeric as x,
             mx.cogs_per_unit::numeric as y
